...
#define NUMBER_OF_EXIT_FRAMES_SUPPORTED 0x04
  ...
  UINT8_TO_STREAM(pp,
                  exitframe == AUTOTRANSACT_POLLING_LOOP_FILTER__NOT_SUPPORTED
                      ? 0
                      : 15);  // Currently the table is limited to 10 entries
  *nbtlv += 1;                // 1 TLV was stored.
...
static bool stpropnci_cb_generate_polling_loop_frame(bool dir_from_upper,
  ...
  int conv_tlv = 0;
  uint32_t curr_ts = 0;
  bool observe_matching = false;

  if (stpropnci_state.observe_per_tech_bitmap == 0 ||
      stpropnci_state.observe_mode_suspended) {
    // observe mode not enabled or suspended, do nothing
    return false;
  }
  ...
  while (1) {
    ...
    if (process) {
      ...
      switch (T) {
        ...
        case FWLOG_T_CERx:
          ...
          if (error != 0) {
            if (IS_HW_54L_OR_MORE_RECENT_FAMILY()) {
              if (error == 0x41) {
                type = TAG_NFC_UNKNOWN;
              } else {
          ...
          //Check if observe_matching_exit_frame is set
          if (stpropnci_state.observe_matching_exit_frame_len != 0) {
            if (stpropnci_state.observe_matching_exit_frame_len == reallen) {
              //0x00: type A
              //0x01: type B
              //0x02: type A short frame
              if (((stpropnci_state.observe_matching_exit_frame_type == 0x00) &&
                   ((flag & 1) == 0) && (type == TAG_NFC_A)) ||
                  ((stpropnci_state.observe_matching_exit_frame_type == 0x01) &&
                   ((flag & 1) == 0) && (type == TAG_NFC_B)) ||
                  ((stpropnci_state.observe_matching_exit_frame_type == 0x02) &&
                   ((flag & 1) == 1) && (type == TAG_NFC_A))) {
                // We have a matching exit frame
                if ((reallen == 1) &&
                    (stpropnci_state.observe_matching_exit_frame[0] ==
                     (payload[reallenidx + 2]))) {
                  observe_matching = true;
                } else if (!memcmp(stpropnci_state.observe_matching_exit_frame,
                                   payload + (reallenidx + 2), availlen)) {
                  observe_matching = true;
                }
              }
            }
    ...
    if (observe_matching) {
      // We had a matching exit frame, stop processing further TLVs
      // process the TLV
      stpropnci_state.observe_mode_suspended = true;
      // Force exiting the while() loop to prevent sending PLF for other
      // TLVs
      break;
    }
...
static bool stpropnci_build_set_custom_polling_cmd(uint8_t *buf,
  ...
  //   len [N+1]
  //    RFU [1B]
  //    RF frame [N=1~16B] -- CRC is added compared with upper motif
  ...
  if (nb_frames) {
    ...
    motiflen = *in++ - 1;  // size of the annotation from JNI
    if (motiflen >= 14) {
      LOG_D(
          "NCI_ANDROID_SET_TECH_A_POLLING_LOOP_ANNOTATION truncated "
          "to fit the CRC");
      motiflen = 13;
    }
    UINT8_TO_STREAM(
        pp,
        1 + motiflen +
            2);  // length. JNI already added 1 to the payload. Spec to check...
