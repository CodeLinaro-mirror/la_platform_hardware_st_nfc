...
static void* message_pump_thr(void* st) {
  ...
  do {
    ...
    while ((m = message_pop_first_except(&stpropnci_state.pumpstate.toSend,
      ...
      if (!m->dir_to_nfcc) {
        if (stpropnci_state.nfc_recovery &&
            ((m->payload[0] != 0x60) || (m->payload[1] != 0x00) ||
             (m->payload[7] != 0x00))) {
          LOG_D("Dropping message 0x%x 0x%x 0x%x", m->payload[0], m->payload[1],
                m->payload[2]);
          discard = true;
        }
      ...
      if (tscmp(stpropnci_state.pumpstate.toAck.next->ts, t) < 0) {
        ...
        if (m->payload[0] == 0x00) {
          ...
          if (stpropnci_state.ts_last_rf_deact_ntf.tv_sec != 0) {
            tssub(&t, 10); // add some margin for processing and transmission time
